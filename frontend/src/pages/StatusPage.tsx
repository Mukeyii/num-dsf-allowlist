...
function StatCard({ icon, color, label, value, detail }: { icon: string; color: string; label: string; value: string | number; detail?: string }) {
  return (
    <div style={{ padding: '16px', background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border)', flex: 1, minWidth: '140px' }}>
      ...
      {detail && <p style={{ fontSize: '10px', color: 'var(--text-muted)', margin: '4px 0 0' }}>{detail}</p>}
    </div>
  );
}
...
export function StatusPage() {
  ...
  const nextExpiry = certs.length > 0
    ? Math.min(...certs.map((c: any) => daysUntil(c.valid_until)))
    : null;

  const statusColor = approval?.status === 'APPROVED' ? '#22c55e'
    : approval?.status === 'PENDING' ? '#f5a623'
    : approval?.status === 'REJECTED' ? '#ef4444' : 'var(--text-muted)';
        ...
        {history.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {history.slice(0, 10).map((req: any, i: number) => {
              const color = req.status === 'APPROVED' ? '#22c55e' : req.status === 'PENDING' ? '#f5a623' : '#ef4444';
              return (
                <div key={req.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '8px', borderRadius: '8px', background: 'var(--bg-hover)' }}>
                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: color, flexShrink: 0 }} />
                  <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)', flex: 1 }}>Request #{history.length - i}</span>
