...
function Section({ id, title, children }: { id: string; title: string; children: React.ReactNode }) {
  return (
    <section id={id} style={{ marginBottom: '32px' }}>
      <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <span style={{ width: '4px', height: '20px', background: '#b01e66', borderRadius: '2px' }} />
        {title}
      </h2>
...
export function AdminHelpPage() {
  ...
  return (
    <div style={{ flex: 1, padding: '32px', overflowY: 'auto', fontFamily: 'Inter, system-ui, sans-serif', maxWidth: '900px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
        <span className="material-symbols-outlined" style={{ fontSize: '28px', color: '#b01e66' }}>help</span>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>{t('adminHelpTitle')}</h1>
          <p style={{ fontSize: '13px', color: 'var(--text-muted)', margin: '2px 0 0' }}>{t('adminHelpSubtitle')}</p>
        </div>
      </div>

      <div style={{ height: '1px', background: 'var(--border)', margin: '20px 0 24px' }} />

      <nav style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '12px 16px', marginBottom: '24px' }}>
        <p style={{ margin: '0 0 8px', fontSize: '11px', fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>{t('adminHelpToc')}</p>
        <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexWrap: 'wrap', gap: '12px', fontSize: '12px' }}>
          {(['approval', 'crossUser', 'audit', 'download', 'certs', 'mtls', 'support'] as const).map(id => (
            <li key={id}><a href={`#${id}`} style={{ color: '#b01e66', textDecoration: 'none' }}>{TOC_LABEL[id]}</a></li>
          ))}
        </ul>
      </nav>
      ...
      <Section id="support" title={t('adminHelpSupportTitle')}>
        ...
        <p style={{ marginTop: '12px', fontSize: '12px', color: 'var(--text-muted)' }}>
          {t('adminHelpDevelopedBy')}: <strong style={{ color: 'var(--text-primary)' }}>Kemal Yildirim</strong> · {t('footerAffiliation')}
        </p>
      </Section>
    </div>
  );
