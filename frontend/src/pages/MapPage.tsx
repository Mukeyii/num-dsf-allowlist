...
export function MapPage() {
  ...
  const selectedOrg =
    filtered.find((o) => o.identifier === selectedId) ??
  ...
  const selectedCluster = useMemo<MapClusterGroup | null>(() => {
    ...
    const members = organizations.filter(
      (o) => cityBucketKey(o.city, o.country_code) === targetKey,
    );
  ...
  return (
    ...
    >
      <MapFilters
        ...
        verbundPills={
          <VerbundPills counts={counts} active={activeVerbunds} onToggle={toggleVerbund} />
        }
      />

      <div style={{ flex: 1, display: 'flex', minHeight: 0, position: 'relative' }}>
        <MapOrgList organizations={filtered} selectedId={selectedId} onSelect={setSelectedId} />
        <div
