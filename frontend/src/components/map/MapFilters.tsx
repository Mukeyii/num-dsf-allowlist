...
export function MapFilters({
  state, onChange, totalCount, visibleCount,
  cityCount, showAllEdges, onToggleShowAllEdges, verbundPills,
}: Props) {
  const { t } = useI18n();
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        ...
        <div style={{ marginLeft: 'auto', fontSize: '11px', color: 'var(--text-muted)' }}>
          {t('mapFilterShowingOfCities', { visible: visibleCount, total: totalCount, cities: cityCount })}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        ...
        <button
          ...
          style={{
            display: 'inline-flex', alignItems: 'center', gap: '6px',
            padding: '4px 10px', borderRadius: '999px',
            border: `1px solid ${showAllEdges ? '#b01e66' : 'var(--border)'}`,
            background: showAllEdges ? '#b01e6622' : 'transparent',
            color: showAllEdges ? '#b01e66' : 'var(--text-secondary)',
            fontSize: '11px', fontWeight: 600, cursor: 'pointer',
            marginLeft: 'auto',
          }}
        >
          <span className="material-symbols-outlined" style={{ fontSize: '14px' }}>polyline</span>
          {t('mapShowAllEdges')}
        </button>
      </div>
    </div>
