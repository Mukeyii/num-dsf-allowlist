...
adminCaBlacklistRouter.get('/', asyncHandler(async (_req, res) => {
  const [blacklist, knownCas] = await Promise.all([
    svc.listBlacklist(),
    svc.listKnownCas(),
  ]);
...
adminCaBlacklistRouter.post('/', asyncHandler(async (req, res) => {
...
adminCaBlacklistRouter.delete('/:id', asyncHandler(async (req, res) => {
