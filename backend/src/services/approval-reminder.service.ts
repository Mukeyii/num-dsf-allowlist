 ...
 * 1. notifyGeckoOnSubmit() – on new approval request
 * 2. notifySiteOnApproval() – after approve/reject
 * 3. runApprovalReminders() – cron: stale pending requests
 */
import { db } from '../db/connection';

const GECKO_EMAILS = (process.env.GECKO_ADMIN_EMAILS || '')
  .split(',').map(e => e.trim()).filter(Boolean);

export async function notifyGeckoOnSubmit(
  requestId: string, instanceId: string, orgIdentifier: string, orgName: string, submittedBy: string
): Promise<void> {
  if (GECKO_EMAILS.length === 0) {
    console.warn('[ApprovalNotify] No GECKO_ADMIN_EMAILS configured');
    return;
  }
  console.log(`[ApprovalNotify] New request from ${orgIdentifier} by ${submittedBy} → notifying GECKO`);
}
