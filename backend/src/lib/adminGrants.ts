...
export function canonicalMessage(email: string, grantedAt: Date | string, grantedByA: string, grantedByB: string): string {
  const iso = typeof grantedAt === 'string' ? new Date(grantedAt).toISOString() : grantedAt.toISOString();
  return `${email.toLowerCase()}|${iso}|${grantedByA.toLowerCase()}|${grantedByB.toLowerCase()}`;
}

export function signGrant(email: string, grantedAt: Date, grantedByA: string, grantedByB: string): string {
...
export function verifyGrant(grant: Pick<AdminGrant, 'email' | 'granted_at' | 'granted_by_a' | 'granted_by_b' | 'signature_hex'>): boolean {
  try {
    const msg = canonicalMessage(grant.email, grant.granted_at, grant.granted_by_a, grant.granted_by_b);
    return crypto.verify('sha256', Buffer.from(msg, 'utf8'), PUBLIC_KEY_PEM, Buffer.from(grant.signature_hex, 'hex'));
  } catch {
    return false;
  }
}
