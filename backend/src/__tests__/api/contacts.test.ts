...
import { app } from '../../app';
import { cleanTestData, seedTestUser, seedOrganization, seedContact, TEST_INSTANCE_ID } from '../helpers/seed';
...
describe('Contacts API', () => {
  ...
  describe('GET /api/v1/instances/:id/contacts', () => {
    it('should return empty array when no contacts', async () => {
      const res = await request(app).get(`/api/v1/instances/${TEST_INSTANCE_ID}/contacts`).set('Authorization', `Bearer ${token}`);
  ...
  describe('POST /api/v1/instances/:id/contacts', () => {
    it('should create a new contact', async () => {
      const res = await request(app).post(`/api/v1/instances/${TEST_INSTANCE_ID}/contacts`).set('Authorization', `Bearer ${token}`)
  ...
  describe('PUT /api/v1/instances/:id/contacts/:cid', () => {
    it('should update an existing contact', async () => {
      ...
      const res = await request(app).put(`/api/v1/instances/${TEST_INSTANCE_ID}/contacts/${contactId}`).set('Authorization', `Bearer ${token}`)
  ...
  describe('DELETE /api/v1/instances/:id/contacts/:cid', () => {
    it('should delete a contact', async () => {
      const contactId = await seedContact();
      const res = await request(app).delete(`/api/v1/instances/${TEST_INSTANCE_ID}/contacts/${contactId}`).set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(200);
      const check = await request(app).get(`/api/v1/instances/${TEST_INSTANCE_ID}/contacts`).set('Authorization', `Bearer ${token}`);
