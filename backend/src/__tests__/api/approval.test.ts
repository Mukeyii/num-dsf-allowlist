...
import { app } from '../../app';
import { cleanTestData, seedTestUser, seedAdminUser, seedOrganization, TEST_INSTANCE_ID, TEST_ADMIN_EMAIL } from '../helpers/seed';
...
describe('Approval API', () => {
  ...
  describe('POST /api/v1/instances/:id/approval/submit', () => {
    it('should submit an approval request', async () => {
      const res = await request(app).post(`/api/v1/instances/${TEST_INSTANCE_ID}/approval/submit`).set('Authorization', `Bearer ${userToken}`);
  ...
  describe('GET /api/v1/instances/:id/approval/status', () => {
    it('should return null when no requests exist', async () => {
      const res = await request(app).get(`/api/v1/instances/${TEST_INSTANCE_ID}/approval/status`).set('Authorization', `Bearer ${userToken}`);
      expect(res.status).toBe(200);
