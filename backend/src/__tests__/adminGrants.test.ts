...
describe('admin grant signing', () => {
  ...
  it('survives MySQL TIMESTAMP round-trip (sub-second truncation)', () => {
    ...
    expect(verifyGrant({ email, granted_at: fromDb, granted_by_a: a, granted_by_b: b, signature_hex: sig })).toBe(true);
    ...
    expect(verifyGrant({ email, granted_at: fromDb, granted_by_a: a, granted_by_b: b, signature_hex: sigSub })).toBe(false);
  });
});
