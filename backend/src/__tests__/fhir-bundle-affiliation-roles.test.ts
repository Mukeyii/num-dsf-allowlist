...
interface CodeBlock { coding?: Coding[] }
interface Affiliation {
  resourceType?: string;
  code?: CodeBlock[];
}
interface FhirEntry { resource?: Affiliation; request?: { url?: string } }
...
describe('OrganizationAffiliation role fidelity', () => {
  ...
  beforeAll(async () => {
    ...
    await db('instances').insert({ id: instanceId, user_id: userId, label: 'roles-test', created_at: new Date() });
    await db('organizations').insert({
      identifier: orgIdentifier, instance_id: instanceId, name: 'Roles Test', active: true,
